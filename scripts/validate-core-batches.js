...
const reviewed = [
  JSON.parse(fs.readFileSync(path.join(ROOT, 'content/core-101-150.source.json'), 'utf8')),
  JSON.parse(fs.readFileSync(path.join(ROOT, 'content/core-151-200.source.json'), 'utf8')),
  JSON.parse(fs.readFileSync(path.join(ROOT, 'content/core-201-250.source.json'), 'utf8')),
  JSON.parse(fs.readFileSync(path.join(ROOT, 'content/core-251-300.source.json'), 'utf8')),
];
...
assert.strictEqual(new Set(planned).size, 300, 'Core 300 plan contains duplicate lexical units');
...
console.log('core plan ok: 100 verified, 200 reviewed with 600 examples, 0 content-only candidates');
