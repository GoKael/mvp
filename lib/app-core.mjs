...
export function lessonSegmentProgress(state, lessonId, segmentId) {
  return state.lessons[lessonId]?.segments?.[segmentId] || {
    understoodAt: 0,
    feynman: '',
    feynmanAt: 0,
    recalledAt: 0,
...
export function startLesson(state, lessonId, now = Date.now()) {
  ...
  state.lessons[lessonId] = { ...lesson, startedAt: lesson.startedAt || now };
  return state;
}
...
export function lessonLearningProgress(state, lesson) {
  ...
  return {
    started: Boolean(state.lessons[lesson.id]?.startedAt),
    understood: segments.filter((segment) => segment.understoodAt).length,
    explained: segments.filter((segment) => segment.feynman).length,
    recalled: segments.filter((segment) => segment.recalledAt).length,
    completed: Boolean(state.lessons[lesson.id]?.completedAt),
  };
}
...
export function setWordStatus(state, wordId, status, now = Date.now()) {
  ...
  if (status === STATUS.LEARNING) {
    next.box = 1;
    next.nextReviewAt = now + 12 * 60 * 60 * 1000;
...
export function progressSummary(words, lessons, state, now = Date.now()) {
  ...
  return {
    ...counts,
    due: dueWords(words, state, now).length,
    completedLessons: lessons.filter((lesson) => state.lessons[lesson.id]?.completedAt).length,
  };
}

export function completeLesson(state, lesson, now = Date.now()) {
  state.lessons[lesson.id] = { ...(state.lessons[lesson.id] || {}), completedAt: now, lastSegment: 3 };
...
export function currentLesson(lessons, state) {
  return lessons.find((lesson) => !state.lessons[lesson.id]?.completedAt) || lessons[0];
}
