...
Core 300 採分批品質閘門：Core `101–300` 已拆成四個 `content/core-*-*.source.json` 編輯來源，每批 50 詞、每詞三例句，品質維持 `reviewed`。`reviewed` 不是正式資料，不得被 UI 載入；只有例句、四個音檔與人工抽聽皆通過後才能升為 `verified`。批次 `rank` 目前是穩定的內容目錄位置，不等於課程解鎖順序；課程必須優先帶入高頻且當下情境必要的詞，例如 `nhiều / rất / cũng`，不可要求使用者依 1–300 線性背誦。
...
目前課程資料已有 `vi / zhTW` 雙語文字與雙語音檔，Today、情境課與句型畫面可透過顯示設定交換主語言；單字與複習不能只把欄位顛倒，因為詞條 ID、詞性、例句音檔與 SRS 進度目前都以越南語為學習目標。

正式支援「越南人學繁中」前必須完成：

- 將詞條改為語言中立 concept ID，兩種語言各自保存文字、詞性與音檔。
- 將複習進度以 `方向 + concept ID` 分開保存；會看懂越南語不代表會主動說出繁中。
- 補齊繁中單字與例句音檔、越南文介面，以及以繁中為答案的題型校驗。
- 先讓課程頁支援方向切換，再遷移單字與 SRS；資料未齊前不顯示半成品切換器。

因此現有架構可重用 UI、路由、播放器與排程演算法，但資料模型只能部分互換，不能安全地一鍵反轉。
