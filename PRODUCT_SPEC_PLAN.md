...
MVP 以網站／PWA 為主。正式學習與複習只使用人工校驗的 Core 100；詞典另外顯示 200 個 `reviewed` 詞供查閱，但不提供發音、狀態或複習操作。首發另包含 10 課越南美食課程與 8 組核心句型。越南人學繁中、Safari Extension、iOS 與 Android 保留在資料模型與後續路線，不阻塞首發。
...
- `#/words`：Core 詞典；目前 100 詞可學、300 詞以「校對中」只讀顯示，逐批擴充至 Core 1000。
...
- `server/data/lexicon.json`（100 個 `verified`＋200 個只讀 `reviewed` 詞典索引）
...
Core 1000 採分批品質閘門：每個 `content/core-*-*.source.json` 固定 50 詞、每詞三例句，品質維持 `reviewed`。`reviewed` 只可進入只讀詞典，不得進入播放、學習狀態、批次標記或複習題庫；只有詞義與例句校對、中越詞頭與三例句共 8 個音檔、50 個孤立詞雙語人工抽聽及完整音檔機器驗證皆通過後才能升為 `verified`。Core `101–300` 的既有越語音檔可沿用，但仍須補齊繁中音檔才可發布。批次 `rank` 是穩定內容目錄位置，不等於課程解鎖順序。
...
1. 每批 50 詞完成繁中詞義、詞性、三例句、八個中越雙語音檔與人工抽聽，逐批從 Core 100 發布到 Core 1000。
