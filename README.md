...
Lexa 是給台灣使用者的越南語學習 PWA：每課 30 秒、三句中越對照，搭配 Core 詞典、主動回憶與本地間隔複習。目前 100 詞已發布，另有 450 詞以「校對中」只讀顯示；下一個內容目標是 Core 1000。
...
正式學習資料位於 `server/data/core.json`、`lessons.json`、`patterns.json` 與 `audio-manifest.json`；`lexicon.json` 是目前 550 詞的查閱索引，只有 `verified` 詞能進入播放、狀態與複習。`core-100.json` 保留為不可變的首發種子。歷史詞表只保留在 `server/data/archive/`，不會直接進入正式 UI。
...
部署 Cloudflare Pages 時，Build command 使用 `npm run build`，輸出目錄使用 `dist`。建置發布正式 Core 100、Core 550 只讀詞典、10 課、8 句型與 manifest 內 500 個正式音檔；其餘未發布音檔、候選 TTS 任務、QA 與製作來源不會公開。完整策略見 [`DEPLOYMENT_PLAN.md`](DEPLOYMENT_PLAN.md)。
